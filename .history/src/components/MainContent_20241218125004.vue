<template>
  <main class="flex-1 p-4 overflow-y-auto bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white">
    ...
    <div v-if="filteredArtists.length > 0" class="relative mt-12">
      ...
      <button @click="fetchArtists(-1)" :disabled="playlistOffset === 0" class="absolute left-0 top-1/2 -translate-y-1/2 bg-gray-800 text-white text-3xl font-bold p-4 rounded-full shadow-2xl hover:bg-green-500 hover:text-white transition-transform transform hover:scale-125 disabled:opacity-50 disabled:cursor-not-allowed">
        &lt;
      </button>
      <div class="flex gap-4 overflow-x-auto scrollbar-hide">
        <div
